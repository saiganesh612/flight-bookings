...
function Payments({ flight, user, userDetails, bookFlight, clearBooking, location, date }) {
  ...
  async function doPayment() {
    ...
    const res = await axios.post(`/api/book-ticket/${data.flightId}`, { price: data.price, travellers: data.travellers, departureDate: date })
  ...
  return (
    <div>
      ...
      <Jumbotron>
        <h4>Pay {fare}</h4>
        <p>
          <Button onClick={() => doPayment()} variant="primary">
            Pay now
          </Button>
        </p>
      </Jumbotron>
