    ...
    if (data.firstName && data.lastName && data.age) {
      if (isNaN(data.age)) {
        dispatch({
          ...
          payload: "Age should be a number.",
        });
        return false;
      } else return true;
    } else {
...
export const storeFlight = (flightId) => {
  ...
  return async (dispatch) => {
    try {
      const res = await axios.get(`/api/fetch-flight/${flightId}`);
...
export const searchFlight = (data) => {
  return async (dispatch) => {
    try {
      const res = await axios.post("/api/search-flights", data);
      console.log(res.data);
      if (res.data.data.length > 0) {
        dispatch({
          type: SEARCH_FLIGHT,
          payload: { flights: res.data.data, date: res.data.date },
        });
        dispatch({
